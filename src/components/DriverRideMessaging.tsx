...
interface DriverRideMessagingProps {
  rideId: string;
  rideStatus: string;
  riderId: string;
  riderName?: string;
...
export default function DriverRideMessaging({ 
  rideId, 
  rideStatus, 
  riderId,
  riderName = 'Rider',
  className 
}: DriverRideMessagingProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      // Count unread (messages from rider since last read)
      if (typedData.length > 0) {
        const riderMessages = typedData.filter(m => m.sender_role === 'rider');
        if (riderMessages.length > 0) {
          const lastRead = lastReadTimestampRef.current;
          if (lastRead) {
            const unread = riderMessages.filter(m => m.created_at > lastRead).length;
            setUnreadCount(unread);
          } else {
            if (isOpen) {
              lastReadTimestampRef.current = riderMessages[riderMessages.length - 1].created_at;
              setUnreadCount(0);
            } else {
              setUnreadCount(riderMessages.length);
            }
          }
        }
      }
    ...
    const channel = supabase
      .channel(`driver-ride-messages-${rideId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload) => {
          ...
          // Update unread count if from rider and sheet is closed
          if (newMsg.sender_role === 'rider' && !isOpen) {
  ...
  useEffect(() => {
    if (isOpen && messages.length > 0) {
      const riderMessages = messages.filter(m => m.sender_role === 'rider');
      if (riderMessages.length > 0) {
        lastReadTimestampRef.current = riderMessages[riderMessages.length - 1].created_at;
      }
      setUnreadCount(0);
